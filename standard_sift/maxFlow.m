...
% return : 残留网络r，最大流totalflow
    ...
    [row,col,~]=size(r);
    if row>col
        r =[r,zeros(row,row-col)];
    elseif row<col
        r =[r;zeros(col-row,col)];
    end
    ...
    while dis(src)<n   %源点标号  
        j=findArc(i,r,dis,n);  %沿着可行弧 i→j 移动
        if j>=0  %存在 i → j 
            ...
            x=reLable(i,r,dis,n);  % 找到i能到达的各个顶点j的最小标号
...
function [dis,num] = rev_bfs(r,dis,num,n)   
...
function  ret = findArc(i,r,dis,n)  
